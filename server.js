...
app.post('/api/invitations', async (req, res) => {
  try {
    ...
    const invCheck = await db.checkCanInvite(fromPlayerId);
    if (!invCheck.ok) {
      return res.status(402).json({ error: invCheck.error, code: invCheck.code || 'LIMIT_INVITES' });
    }
...
app.post('/api/leagues', async (req, res) => {
  try {
    ...
    if (result.error) return res.status(result.code === 'LIMIT_BOXES' ? 402 : 400).json({ error: result.error, code: result.code });
...
app.post('/api/seeking', async (req, res) => {
  try {
    ...
    if (result.error) return res.status(result.code === 'LIMIT_SEEKING' ? 402 : 400).json({ error: result.error, code: result.code });
...
app.post('/api/payments/checkout', async (req, res) => {
  try {
    ...
    if (!mp.ok) {
      const errText = await mp.text().catch(() => '');
      console.error('MP preference error:', mp.status, errText.slice(0, 300));
...
app.get('/api/payments/confirm', async (req, res) => {
  try {
    const { payment_id, player_id } = req.query;
    if (!payment_id) return res.status(400).json({ error: 'payment_id requerido' });
    const result = await db.confirmPaymentFromReturn(String(payment_id), player_id);
    if (!result.ok) return res.status(400).json({ error: result.error, status: result.status });
    res.json(result);
  } catch (err) {
    res.status(500).json({ error: 'Error interno' });
  }
